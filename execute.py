...
def parseRotation(rotation: np.ndarray, to: np.ndarray | str) -> np.ndarray:        
    ...
    splited = to.split()
    result_vec[0] = Mth.wrapDegrees(rotation[0] + np.float32(splited[1:] if len(splited) > 1 else 0) if splited[0] == '~' else np.float32(splited[0]))
    result_vec[1] = np.float32(np.clip(rotation[1] + np.float32(splited[1:] if len(splited) > 1 else 0) if splited[1] == '~' else np.float32(splited[1]), -90.0, 90.0))
    return result_vec
